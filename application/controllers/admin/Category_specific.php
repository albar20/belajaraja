<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class category_specific extends CI_Controller {
		...
		public function index()
		{
			...
			$data['title'] 		= 'Halaman Daftar category_specific | '.$judul;
			$data['heading'] 		= "category_specific";
			$data['page']			= 'admin/category_specific/page_list';
		...
		{			
			$user_id 				= $this->session->userdata('id_user');
			$judul					= $this->model_utama->get_detail('1','setting_id','setting')->row()->website_name;
			$data['title'] 			= 'Halaman Tambah category_specific | '.$judul;
			$data['heading'] 		= 'Add category_specific List';
			$data['form_action'] 	= site_url('admin/category_specific/add_process');
			$data['page']			= 'admin/category_specific/page_form';
		...
		function add_process()
		{
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				redirect('admin/category_specific/add', 'refresh');
			}
			else
			{
				$this->load->view($this->admin_template, $data);
		...
		function delete($kode)
		{
			...
			redirect('admin/category_specific');
		}
		...
		function update($kode)
		{
			$user_id 				= $this->session->userdata('id_user');
			$judul					= $this->model_utama->get_detail('1','setting_id','setting')->row()->website_name;
			$data['title'] 		= 'Halaman Ubah category_specific | '.$judul;
			$data['heading'] 		= 'Update category_specific';
			$data['form_action'] 	= site_url('admin/category_specific/update_process/'.$kode);
			$data['query_tabel']	= $this->db->query("select * from category_product");
			$data['primary_key']	= $this->db->query("SHOW KEYS FROM category_product WHERE Key_name = 'PRIMARY'")->row();	
		...
		function update_process($kode)
		{
			...
			$data['title'] 			= 'Halaman Ubah category_specific | '.$judul;
			$data['heading'] 		= 'Update category_specific';
			$data['form_action'] 	= site_url('admin/category_specific/update_process');
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				$this->model_utama->update_data($this->session->userdata('kd_update'),'subcategory_specific_information_id','category_specific_information',$update_data);
				...
				redirect('admin/category_specific/update/'.$this->session->userdata('kd_update'));
			}
			else
			{
