	...
	<div class="blog-post-wrapper container blog_single">            
        <div class="row">
            <div class="blog-entry col-md-8 col-sm-8 col-xs-12">                 
                <article class="post">
                    <h2 class="title post_title"><?php echo ucwords($blog->blog_title)?></h2>
                    <div class="meta">
                        <ul class="meta-list list-inline">                                       
                        	<li class="post-time date updated">
                                <i class="fa fa-calendar"></i> 
                                <span class="post_date"><?php echo date('d F Y',strtotime($blog->create_date)); ?></span>
                            </li>
                        	<!-- <li class="post-author"><i class="fa fa-user"></i> <a href="#">James Lee</a></li> -->
                        	<li class="post-comments-link">
                    	        <a href="#comment-area">
                                    <i class="fa fa-comments"></i> <?php echo $comment_count > 0 ? $comment_count .' comments' : 'No Comment' ?></a>
                    	    </li>
                    	</ul><!--//meta-list-->                           	
                    </div><!--meta-->
                    <div class="content">
                        <?php if($blog->blog_picture != '') : ?><p class="post-figure post_picture_wrapper">
                            <img class="img-responsive post_picture" src="<?php echo base_url()?>uploads/blog/<?php echo $blog->blog_slug?>/<?php echo $blog->blog_picture?>" alt="<?php echo ucwords($blog->blog_title)?>" />
                        </p><!--//post-figure--><!-- post_picture_wrapper --><?php endif;?>
                        <div class="post_description"><?php echo $blog->blog_description; ?></div><!-- post_description -->                         
                    </div>
                    ...
                    <nav class="post-nav">
						<?php 
                            $prev = $previous->result_array();
                            if( count($prev) > 0  ):
                        ?>
                        <span class="nav-previous">
                            <a href="<?php echo base_url().$segment_1.'/'.$prev[0]['blog_slug'] ?>" rel="prev">
                                <i class="fa fa-long-arrow-left"></i>Previous</a></span>
                        <?php endif; ?>
                        <?php 
                            $next = $next->result_array();
                            if( count($next) > 0  ):
                        ?>
						<span class="nav-next"><a href="<?php echo base_url().$segment_1.'/'.$next[0]['blog_slug'] ?>" rel="next">Next
    				...
    				<div id="comment-area" class="comment-area">
    				    <div class="comment-container">        				    
        				    <div class="comment-list">
                                ...
                                <h3 class="title">
                                    <?php echo $comment_count > 0 ? $comment_count .' comments' : 'No Comment' ?>
                                </h3>
                                <?php if($comment_list->num_rows() > 0) : ?>
                                <div class="comment_list_wrapper">
                				    <?php foreach($comment_list->result() as $row) : ?>
                                    <div class="comment-item depth-1 parent comment_list_inside">
                    				    <div class="comment-item-box">
                        				    <div class="comment-author">
                            				    <img class="img-responsive" src="assets/images/comment/user-1.jpg" alt="" />
                        				    </div><!--//comment-author-->
                        				    <div class="comment-body">
                            				    <cite class="name comment_name_on_list"><?php echo ucwords($row->comment_name)?></cite>
                            				    <p class="time comment_date_on_list"><?php echo date('d F Y',strtotime($row->create_date)); ?></p>
                            				    <div class="content comment_content_on_list"><?php echo strip_tags($row->comment_content); ?><!--//content--><!-- comment_content_on_list -->  
                            				    <!-- <a class="comment-reply-link btn btn-cta btn-cta-secondary" href="#">Reply</a> -->
                        				    </div><!--//comment-body-->          
                    				    </div><!--//comment-box--> 
                				    </div><!--//comment-item depth-1--><!-- comment_list_inside -->
                                    <br/>
                                    <?php endforeach; ?>
                                </div><!-- comment_list_wrapper -->
                                <?php endif; ?>    
        				    </div><!--comment-list-->
    				    ...
    				    <div class="comment-form-container">
        				    ...
        				    <form id="comment-form" class="comment-form form" method="post" action="#">                   
                                <div class="row">                                                                                                                                                                     
                                    <div class="col-md-6 col-sm-6 col-xs-12 form-group form_error_comment_name <?php if(form_error('comment_name') != '') { echo 'has-error'; } ?>">
                                        <label class="sr-only" for="cname">Name</label>
                                        <input type="text" class="form-control" id="cname" name="comment_name" placeholder="Name" minlength="2" required value="<?php echo set_value('comment_name', isset($default['comment_name']) ? $default['comment_name'] : ''); ?>" />
<?php echo form_error('comment_name', '<span class="help-block" style="background-color:yellow"><i class="fa fa-warning"></i>', '</span>'); ?>                                    </div>                    
                                    <div class="col-md-6 col-sm-6 col-xs-12 form-group form_error_comment_email <?php if(form_error('comment_email') != '') { echo 'has-error'; } ?>">
                                        <label class="sr-only" for="cemail">Email</label>
                                        <input type="email" class="form-control" id="cemail" name="comment_email" placeholder="Email" required value="<?php echo set_value('comment_email', isset($default['comment_email']) ? $default['comment_email'] : ''); ?>" />
<?php echo form_error('comment_email', '<span class="help-block" style="background-color:yellow"><i class="fa fa-warning"></i>', '</span>'); ?>                                    </div>
                                    <div class="col-md-12 col-sm-12 col-xs-12 form-group form_error_comment_content <?php if(form_error('comment_content') != '') { echo 'has-error'; } ?>">
                                        <label class="sr-only" for="ccomment">Leave a Comment</label>
                                        <textarea class="form-control" id="ccomment" name="comment_content" placeholder="Leave your comment" rows="12" required value="<?php echo set_value('comment_content', isset($default['comment_content']) ? $default['comment_content'] : ''); ?>" /></textarea value="<?php echo set_value('comment_content', isset($default['comment_content']) ? $default['comment_content'] : ''); ?>" />
<?php echo form_error('comment_content', '<span class="help-block" style="background-color:yellow"><i class="fa fa-warning"></i>', '</span>'); ?>                                    </div>
            ...
            <?php //$this->load->view("main/devstudio/blog/sidebar"); ?>             
        </div><!--//row-->
